# [.agents/prd.md] / <Feature or Module Name>

## 1. Overview
Problem summary + why it matters
Target users
Core success criteria

## 2. Business & User Context
Business goals
Target persona(s)
User needs
Competition & differentiation

## 3. Functional Requirements
List of features with clear, measurable acceptance criteria

## 4. Non-Functional Requirements (NFR)
Performance (e.g., latency)
Security & privacy
Scalability
Accessibility

## 5. UX & Interaction Specs
User flows (happy + edge cases)
Mockup references
Feedback states (error, success)

## 6. AI / Model Expectations
Model type & configuration
Prompt examples
Evaluation rubric

## 7. Risks & Mitigations
Technical risk
AI behavior risk
UX risk

## 8. Success Metrics & KPIs
Quantitative metrics
Qualitative user outcomes

## 9. Dependencies
Tech, design, data, third-party

## 10. Revision History
Who changed what, why, when
